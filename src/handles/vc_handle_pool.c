...
void    vc_handle_pool_create(vc_handle_pool *pool, u64 initial_chunk_count, u64 managed_size)
{
    ...
    u64 initial_size = ( managed_size + sizeof(vc_handle_pool_chunk_header) - sizeof(u64) ) * initial_chunk_count;
    ...
    pool->head_id         = 0;
    pool->chunk_size      = sizeof(vc_handle_pool_chunk_header) + managed_size;
}
...
u32     vc_handle_pool_alloc(vc_handle_pool   *pool)
{
    ...
    // Dereference pool
    vc_handle_pool_chunk_header *new_hdr = pool->memory + (new_id * pool->chunk_size);
    if(new_hdr->used)
    {
...
void   *vc_handle_pool_deref(vc_handle_pool *pool, u32 id)
{
    ...
    {
        vc_error("Handle counter mismatch with managed ressource. This indicate a dangling access.");
        return NULL;
    }
